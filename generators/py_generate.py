...
MY_SIMPLE_CHAT_INSTRUCTION = "# You are an AI countdown player. You will be given 5 numbers. Your goal is to use the first four numbers to reach the fifth number by performing basic arithmetic operations (+, -, *, /). You can use each number only once. Write the arithmetic expression that reaches the target number using the given numbers. If it is not possible to reach the target number using the given numbers, return 'impossible'."
MY_SIMPLE_COMPLETION_INSTRUCTION = "Write the progress you made in the countdown game."
MY_REFLEXION_CHAT_INSTRUCTION = " You are an AI countdown player. You will be given your previous arithmetic expression."
MY_REFLEXION_COMPLETION_INSTRUCTION = "You are an AI countdown player. You will be given your previous arithmetic expression, a series of unit tests results, and your self-reflection on your previous arithmetic expression. Write your full arithmetic expression (restate the numbers and operators)."
USE_MY_CODEBLOCK_INSTRUCTION = "Use a code block to write your response. For example:\n```raw\n1 + 2 + 3 + 4\n```"
MY_REFLEXION_FEW_SHOT_ADD = '''Example 1:
[previous arithmetic expression]:
```raw
1 + 2 + 3 + 4 = 4
```
[reflection on previous arithmetic expression]:
The arithmetic expression failed the test cases where the target number is 11. The issue arises because the code does not reach the target number using the given numbers. To fix this issue, we should change the arithmetic expression to reach the target number 11 using the given numbers. This can be done by modifying the operators and the order of the numbers in the expression. This will ensure that the function returns the correct output for the given input.\n[improved arithmetic expression]:
```raw
1 + 2 - 3 + 4 = 4
```'''
    ...
    def func_impl(
        ...
        if strategy == "mcts":
            return generate_with_accumulated_context(
                ...
                # add_code_block=lambda x: add_code_block(x, "python"),
                reflection_chat_instruction=MY_REFLEXION_CHAT_INSTRUCTION,
                reflection_few_shot=MY_REFLEXION_FEW_SHOT_ADD,
                simple_chat_instruction=MY_SIMPLE_CHAT_INSTRUCTION,
                reflection_completion_instruction=MY_REFLEXION_COMPLETION_INSTRUCTION,
                simple_completion_instruction=MY_SIMPLE_COMPLETION_INSTRUCTION,
                code_block_instruction=USE_MY_CODEBLOCK_INSTRUCTION,
                parse_code_block=lambda x: parse_code_block(x, "python"),
                add_code_block=lambda x: add_code_block(x, "python"),
            )
        else:
